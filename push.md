...
- `git push <remote> <local_branch>:<remote_branch>` – добавить изменения из локальной ветки `<local_branch>` и переместить ветку удаленного репозитория `<remote_branch>`
...
- `git push --force-with-lease` – выполнить `push`, если является предком или удаленная ветка не сдвигалась (использовать вместо `-f`)
